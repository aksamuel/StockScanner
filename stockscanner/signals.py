...
def generate_signal(df):
    ...
    if close > ma20 > ma50 > ma200 and macd > macd_signal and 50 <= rsi <= 70:
        return _prefer_emoji("🟢 Strong Uptrend", "Strong Uptrend")

    if close >= ma20 * 0.98 and close <= ma20 * 1.02 and ma20 > ma50 > ma200:
        return _prefer_emoji("🟢 Pullback to 20 MA", "Pullback to 20 MA")

    if close >= ma50 * 0.98 and close <= ma50 * 1.02 and ma50 > ma200:
        return _prefer_emoji("🟢 Pullback to 50 MA", "Pullback to 50 MA")

    high20 = df["High"].tail(20).max()

    if close >= high20 * 0.99 and macd > macd_signal:
